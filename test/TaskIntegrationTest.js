...
describe("Setting taskProxy and taskMemberProxy in TaskMemberManager and TaskManager contracts", async () => {
  it("Should correctly assign proxies addresses to contracts", async () => {
    ...
    expect(await taskManager.getTaskMemberProxy()).to.equal(
...
describe("The whole workflow of a single Task", async () => {
  it("Should create a Task with a specified 'taskId' and other data and get it using 'taskId'", async () => {
    ...
    await expect(taskManager.getTask("weirdId")).to.be.revertedWith(
      "A task with this ID does not exist"
    );
    ...
    await expect(
      taskManager.createTask(diffDaoContract, taskId, diffPrize, diffPercentage)
    ).to.be.reverted;
    // Check if it throws the right message
    await expect(
      taskManager.createTask(diffDaoContract, taskId, diffPrize, diffPercentage)
  ...
  it("Should add Workers and Reviewers to a Task and remove them from the Task, which is only in PENDING stage", async () => {
    ...
    await expect(
      memberManager.connect(reviewers[0]).removeMember(taskId)
    ...
    await expect(
      memberManager.connect(reviewers[0]).addReviewer(taskId)
    ...
    await expect(memberManager.connect(reviewers[0]).addWorker(taskId)).to.be
      .reverted;

    // Should throw an exception, as the Task doesn't have 1 Reviewer and 1 Worker yet
    await expect(taskManager.beginTask(taskId)).to.be.revertedWith(
      "In order to begin a task, it should have at least 1 reviewer and 1 worker"
    );
    ...
    await expect(
      memberManager.connect(workers[0]).addReviewer(taskId)
    ).to.be.revertedWith("You have already been assigned to this task");
    ...
    await expect(memberManager.connect(workers[0]).addWorker(taskId)).to.be
      .reverted;
    ...
    await expect(
      memberManager.connect(reviewers[2]).addReviewer(taskId)
    ).to.be.revertedWith("You can't do this action at current Task stage");
    // Should throw an exception, as users can't apply for any position after a Task has begun
    await expect(memberManager.connect(workers[2]).addWorker(taskId)).to.be
      .reverted;
  });
  ...
  it("Should let a taskOwner begin a Task if at least 1 Reviewer and 1 Worker were already assigned", async () => {
    ...
    await expect(memberManager.connect(workers[0]).addReviewer(taskId)).to.be
      .reverted;
    // Check if it throws the right message
    await expect(
      memberManager.connect(workers[0]).addReviewer(taskId)
    ...
    await expect(taskManager.connect(workers[2]).beginTask(taskId)).to.be
      .reverted;
    // Check if it throws the right message
    await expect(
      taskManager.connect(workers[2]).beginTask(taskId)
  ...
  it("Should allow a taskOwner to delete a Task if it was not started yet", async () => {
    ...
    await expect(taskManager.connect(reviewers[0]).removeTask(taskId)).to.be
      .reverted;
    ...
    await expect(getTaskData(taskManager, taskId)).to.be.revertedWith(
      "A task with this ID does not exist"
    );
    // const updatedReviewers = await getTaskReviewers(taskManager, taskId);
    // expect(0).to.equal(updatedReviewers.length);
    ...
    await expect(taskManager.removeTask(taskId2)).to.be.revertedWith(
      "You can't do this action at current Task stage"
    );
  });
